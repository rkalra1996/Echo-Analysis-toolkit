...
export class ChartComponent implements OnInit {
 ...
 private d3SimpleGraph(svg) : void{
  ...
  // this.svg = d3.select('#canvas').append('svg').attr('width', this.width).attr('height', this.height);
 ...
 animate(index?:number) {
  ...
  let blinker = window.setInterval( () => {
   ...
   if(new Date().getTime() - startTime > 5000){
     ...
     if (nextEl !== undefined){
        console.log('next element to use is ', nextEl);
      ...
      this.sharedSrvc.sendActiveNode('Finished');
      return
    };
 }
  }, 500);
